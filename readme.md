...
已恢复 2026 年 9 月 17 日使用的原版模板（`c9e2fb5`）：包含原版封面、详情页和结尾卡，保留当前新闻数据。

页面代码更新仍通过 `deploy-site.yml` 自动部署；更新渲染器或交互脚本时，同步调整 `index.html` 中的版本参数以避开旧缓存。
...
- 点击首页日期栏或归档日期下的「X 四图打包」，预览后保存单张或下载 ZIP。解压后按文件名顺序将 PNG 上传到同一条 X 帖子。
- 使用独立的 `site/poster-x.js` 排版：10 条新闻按 **3＋3＋2＋2** 分成四张，不额外生成封面或尾卡；小红书的原版模板保持不变。
- 每张 1200×1600，展示连续新闻序号、标题、核心事实、已有的必要提示和来源；每张图片标注日期、新闻范围及页码。超长字段会以省略号提示，完整内容仍可在网站阅读。
- 支持 1—12 条的日报，均分为最多四张且没有空白页；超过 12 条会明确提示，不静默丢弃新闻。图片超过 5MB 时停止打包并提示。
- 文件名为 `拾闻_日期_X_01.png` 等，ZIP 为 `拾闻_日期_X图片.zip`。不连接 X 账号、不自动发帖、不调用额外 AI；只复用 PNG / ZIP 下载工具。
- X 的四图及文件限制参考：[X 官方图片说明](https://help.x.com/en/using-x/posting-gifs-and-pictures)。
