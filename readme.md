...
- 卡片信息结构：分类 → 标题 → 一句话总结 → 发生了什么 → 与读者相关的提问 → 这对你意味着什么（三条）→ 注意提示 → 来源与页码
- 详情页分类、总结和提示使用无独立底块的橙色文字（`#c94b2b`）；统一无衬线字体、500 字重和 1px 字距，分类 22px，总结/提示标签 24px。总结与提示共用左对齐标签列及正文起点，背景沿用所在区域。
...
| `means` | 「这对你意味着什么」三条，每条 10-18 字，落到钱／工作／物价／出行 |
| `note` | 不确定性或前提提示，15-30 字 |

改写这一轮失败不会阻塞主流程：Python 侧 `_derive_card()` 与前端 `getCard()` 都会从已有的标题、摘要、入选理由降级推导，历史数据（改动之前的日报）也能正常出图。降级推导出的卡片会标记 `derived`，此时影响区标题改为「为什么值得关注」，并使用通用提问兜底。
