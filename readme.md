...
- 整天导出：日期分组标题右侧「🖼 整天打包」，得到 `封面 + 10 张内容` 共 11 张
- 预览弹窗内可左右翻页（支持 ← → 方向键、Esc 关闭），单张保存或打包成 ZIP 下载

**技术细节**

- 规格 1080×1440（3:4），小红书推荐比例；纯 Canvas 2D 绘制，无第三方依赖
- 排版做了中文优化：按「原子」断行（西文数字不拆开）、优先在标点/空格处换行避免拆词、标题自动缩字号、固定网格布局避免信息区跳动
- ZIP 采用 store 模式手写实现（含 CRC32），PNG 本身已压缩，无需二次压缩，因此不引入 JSZip
- 大字详情页：分类 → 两行以内的大标题 → 一段事实 → 两条影响 → 必要提示 → 来源与页码；不再重复展示副标题、总结和提问。旧数据的第三条影响不在图片中展示，原始新闻数据不改动。
- 标题 86px（长标题最低 68px）、事实与影响正文 42px、正文行距 60–62px、提示 30px。取消大序号侧栏与多层胶囊容器，让文字使用整行宽度；历史超长字段按行数省略，不把正文缩成小字。
- 封面保持原有设计；详情页分类与提示延续橙色文字、暖白底的克制样式。
- `main` 分支上的页面代码更新会通过 `deploy-site.yml` 自动部署现有 `site/`，无需重新抓取新闻或调用 AI；日报数据更新仍使用原有日报部署流程。渲染器更新时同步更新 `index.html` 中 `poster.js` 的版本参数，避免命中旧缓存。
...
| `what` | 发生了什么，30-45 字，客观事实带数字与时间 |
| `question` | 历史兼容字段，不再生成或展示 |
| `means` | 「这对你意味着什么」两条，每条 10-16 字，落到钱／工作／物价／出行 |
| `note` | 不确定性或前提提示，12-24 字 |

改写这一轮失败不会阻塞主流程：Python 侧 `_derive_card()` 与前端 `getCard()` 都会从已有的标题、摘要、入选理由降级推导，历史数据（改动之前的日报）也能正常出图。降级推导出的卡片会标记 `derived`，此时影响区标题改为「为什么值得关注」。两条影响的新格式与旧版三条输入均可读取。
